...
\begin{tabularx}{\textwidth}{p{3.2cm} p{4.2cm} X l c}
\toprule
...
\begin{minipage}[c]{0.54\textwidth}
    ...
    An interactive classroom card role-play experience where students adopt roles (Algorithm, Content Creator, Fact-Checker) and evaluate headlines as Credible or Misleading \emph{before} red flags and XP debriefs unlock.
...
\begin{minipage}[c]{0.54\textwidth}
    ...
    An illustrative analytics dashboard for educators to visualize cohort telemetry, tracking time-to-echo-chamber and behavioral pledges without PII. \emph{(Note: Capture shows active cohort initialization).}
...
\textbf{Tech Stack \& Architecture:} Built using pure HTML5, Vanilla JavaScript, and CSS. The zero-dependency frontend ensures zero supply-chain vulnerabilities, zero build tool breaking changes, and maximum long-term operational stability. Hosting is free via GitHub Pages/Vercel/Render. Anonymous aggregate telemetry uses Firebase Realtime Database (free tier supports up to 100 concurrent connections, managed via client-side batching and local caching to easily accommodate classroom cohorts at \textbf{\$0 monthly cost}).
